...
function boot (env, language) {
  ...
  function setupConnect (ctx, next) {
    ...
    // Patch nightscout-connect librelinkup.js directly on disk to fix:
    // 1. Missing Account-Id header (required since LibreLinkUp API v4.11+)
    // 2. Crash when last_known is null (issue #27)
    // 3. Crash when graphData is null
    try {
      var fs = require('fs');
      ...
      if (!src.includes('LLU_PATCHED')) {
        try { fs.chmodSync(librelinkupPath, 0o644); } catch(ce) { console.log('LLU PATCH: chmod failed:', ce.message); }
        // Fix 1: Add Account-Id header in sessionFromAuth
        src = src.replace(
          "var token = auth.data.authTicket.token;\n      var headers = {\n        'Authorization': [ 'Bearer', token ].join(' ')\n      };",
          "var token = auth.data.authTicket.token;\n      var userId = (auth.data.user || {}).id || '';\n      var accountId = require('crypto').createHash('sha256').update(userId).digest('hex');\n      console.log('LLU PATCH: Account-Id computed');\n      var headers = {\n        'Authorization': [ 'Bearer', token ].join(' '),\n        'Account-Id': accountId\n      };"
        );

        // Fix 2: Null-safe last_known.entries access
        src = src.replace(
          'var last_updated = last_known.entries;',
          'var last_updated = last_known ? last_known.entries : null;'
        );
        ...
        // Fix 4a: Store accountId in session object so graph requests can use it
        src = src.replace(
          "var result = connections[0];\n        result.authTicket = auth.data.authTicket;",
          "var result = connections[0];\n        result.authTicket = auth.data.authTicket;\n        result._accountId = accountId;"
        );

        // Fix 4b: Add Account-Id to graph request (required since LibreLinkUp API v4.11+)
        src = src.replace(
        ...
        src = '// LLU_PATCHED\n' + src;
        fs.writeFileSync(librelinkupPath, src);
        delete require.cache[librelinkupPath];
        console.log('LLU PATCH: librelinkup.js patched on disk successfully');
      } else {
        console.log('LLU PATCH: already patched, skipping');
      }
    } catch(e) {
      console.log('LLU PATCH: Failed to patch librelinkup.js:', e.message);
    }
