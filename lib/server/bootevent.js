...
function boot (env, language) {
  ...
  function setupConnect (ctx, next) {
    ...
    // Patch nightscout-connect librelinkup to add Account-Id header (required since LibreLinkUp API v4.11+)
    // Account-Id = SHA256(user.id) from login response
    try {
      var crypto = require('crypto');
      var librelinkupPath = require.resolve('nightscout-connect/lib/sources/librelinkup');
      var originalLinkUpSource = require(librelinkupPath);

      function patchedLinkUpSource(opts, axios) {
        var accountId = null;
        var patchedAxios = Object.assign(Object.create(Object.getPrototypeOf(axios)), axios);
        patchedAxios.create = function(config) {
          var instance = axios.create(config);
          var origPost = instance.post.bind(instance);
          var origGet = instance.get.bind(instance);
          instance.post = function(url, data, reqConfig) {
            return origPost(url, data, reqConfig).then(function(response) {
              try {
                var userId = response.data.data.user.id;
                if (userId) {
                  accountId = crypto.createHash('sha256').update(userId).digest('hex');
                  console.log('LLU PATCH: Account-Id set for user', userId.substring(0, 8) + '...');
                }
              } catch(e) { /* ignore */ }
              return response;
            });
          };
          instance.get = function(url, reqConfig) {
            if (accountId) {
              reqConfig = reqConfig ? JSON.parse(JSON.stringify(reqConfig)) : {};
              reqConfig.headers = reqConfig.headers || {};
              reqConfig.headers['Account-Id'] = accountId;
            }
            return origGet(url, reqConfig);
          };
          return instance;
        };
        var impl = originalLinkUpSource(opts, patchedAxios);
        // Fix crash when last_known is null on first run (nightscout-connect issue #27)
        // Fix crash when graphData is null in payload
        var origTransform = impl.transformGlucose;
        impl.transformGlucose = function(payload, last_known) {
          try {
            if (payload && payload.data && !payload.data.graphData) {
              payload = Object.assign({}, payload, {
                data: Object.assign({}, payload.data, { graphData: [] })
              });
            }
          } catch(e) { /* ignore */ }
          return origTransform(payload, last_known || { entries: null });
        };
        return impl;
      }
      patchedLinkUpSource.validate = originalLinkUpSource.validate;
      require.cache[librelinkupPath].exports = patchedLinkUpSource;
      console.log('LLU PATCH: nightscout-connect librelinkup patched successfully');
    } catch(e) {
      console.log('LLU PATCH: Failed to patch nightscout-connect:', e.message);
    }
